...
@implementation AutoTestProfile
...
- (void)cbTimePlay:(SSlot*)s {
    ...
    AutoTestAction* action = [_actions objectAtIndex:tm.countSteps def:nil];
    if (action == nil) {
        [tm stop];
        return;
    }
...
- (void)cbClicked:(SSlot*)s {
    ...
    AutoTestAction* action = [AutoTestAction temporary];
...
@interface AutoTestSuite ()

@property (nonatomic, retain) AutoTestProfile *recordingProfile;

@end

@implementation AutoTestSuite
...
- (AutoTestProfile*)record {
    AutoTestProfile* p = [AutoTestProfile temporary];
