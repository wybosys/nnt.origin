...
- (void)foundationInit {
    ...
    kIOS10Above = kIOSMajorVersion >= 10;
    kIOS9Above = kIOSMajorVersion >= 9;
    ...
    if (kIOS8Above) {
        kUIScreenBounds = scr.nativeBounds;
        kUIScreenScale = scr.nativeScale;
    } else {
    ...
    if (kUIApplicationSize.width > 320)
        kUIDpFactor = kUIApplicationSize.width / 320;
    ...
    // 加载自动测试环境
    DEVELOP_EXPRESS([QCTestingSuite Launch]);
    
    // 初始化图片缓存目录
    SDWebImageManager* manager = [SDWebImageManager sharedManager];
    [manager.imageCache makeDiskCachePath:[[FSApplication shared] dirCache:@"images"]];
...
- (void)presentModalViewController:(UIViewController *)modalViewController animated:(BOOL)animated {
    [self.topmostViewController presentViewController:modalViewController animated:animated completion:nil];
}
...
- (void)dismissModalViewControllerAnimated:(BOOL)animated {
    [self.topmostViewController dismissViewControllerAnimated:animated completion:nil];
}
...
- (void)dismissAllModalViewControllersAnimated:(BOOL)animated {
    ...
    [rt dismissViewControllerAnimated:animated completion:nil];
}
