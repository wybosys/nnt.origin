...
- (void)onLoaded {
    ...
    [self.pagedView addSubview:BLOCK_RETURN({
        _pageControl = [UIPageControlExt temporary];
...
- (void)onViewLayout {
    [super onViewLayout];
    UIVBox* box = [UIVBox boxWithRect:self.view.rectForLayout];
    [box addFlex:1 toView:nil];
    [box addPixel:30 toView:_pageControl];
    [box apply];
}
...
- (void)pagedView:(UIPagedView *)view didDisplayPage:(id)page atIndex:(NSInteger)idx {
    [super pagedView:view didDisplayPage:page atIndex:idx];
    _pageControl.numberOfPages = _thumbs.count;
