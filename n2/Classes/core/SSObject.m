...
- (void)setEps:(float)fps {
    _eps = fps;
    
    if (_epsWaiting == nil)
        _epsWaiting = [[NSAtomicCounter alloc] init];
}
...
- (SSignals*)emit:(NSString *)sig withVariant:(Variant *)variant withTunnel:(SSlotTunnel*)tunnel {
    ...
    [slots_dup foreach:^BOOL(SSlot *each) {
        ...
        if (each.eps)
        {
            if (each.origin.epsWaiting) {
                if (SIGNALSLOT_VERBOSE)
                    NOTI("对象 %s 信号 %s 等待冷却", objc_getClassName(_owner), sig.UTF8String);
                return YES;
            }
            
            each.origin.epsWaiting = YES;
            //DISPATCH_DELAY_BEGIN(1 / each.fps)
            //each.source.fpswaiting = NO;
            //DISPATCH_DELAY_END
            [each.origin performSelector:@selector(clearEpsWaiting) withObject:nil afterDelay:(1 / each.eps)];
        }
        else if (sigset.eps)
        {
            if (sigset.epsWaiting.value) {
                if (SIGNALSLOT_VERBOSE)
                    NOTI("对象 %s 信号 %s 等待冷却", objc_getClassName(_owner), sig.UTF8String);
                return YES;
            }
            
            [sigset.epsWaiting add];
            [sigset.epsWaiting performSelector:@selector(sub) withObject:nil afterDelay:(1 / sigset.eps)];
        }
