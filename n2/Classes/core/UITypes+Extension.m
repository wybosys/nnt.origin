...
NSString* kUIImageHighlightSuffix = @"_hl";
...
- (void)setCenterFrame:(CGRect)centerFrame {
    [self setFrame:centerFrame anchorPoint:kCGAnchorPointCC];
}
...
- (void)addSub:(id)obj {
    if ([obj isKindOfClass:[UIView class]]) {
        ...
        FATAL("%s 不是一个 View 或 ViewController", objc_getClassName(obj));
    }
}
...
NSOBJECT_DYNAMIC_PROPERTY_EXT(UIScrollView, identifierBottom,, setIdentifierBottom, {
    ...
    [self directAddSubview:val];
    if (self.identifierBottom.shouldAdjustPullInsets) {
        UIEdgeInsets ei = self.edgeInsetsForPull;
        ei.bottom = [val toggleValue];
...
- (void)__scroll_identifier_toggled {
    if (self.identifierTop.workState == kNSWorkStateWaiting) {
        if (self.identifierTop.shouldAdjustPullInsets) {
    ...
    if (self.workState == kNSWorkStateDone) {
        ...
        if (self.identifierBottom.workState == kNSWorkStateDoing) {
            ...
            if (self.identifierBottom.shouldAdjustPullInsets) {
                [UIView animateWithDuration:kCAAnimationDuration animations:^{
                    UIEdgeInsets ei = self.edgeInsetsForPull;
                    ei.bottom = self.identifierBottom.toggleValue;
                    self.edgeInsetsForPull = ei;
                }];
            }
        }
...
- (CGFloat)tableViewExt:(UITableViewExt*)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cell isKindOfClass:[UITableViewCellExt class]])
    {
        ...
        if (_horizon) {
            [sz swap];
            if (sz.height)
                sz.height += CGPaddingWidth(cellext.paddingEdge);
        } else {
            if (sz.height)
...
- (BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
    ...
    [self.dynAnimator.behaviors enumerateObjectsUsingBlock:^(UIAttachmentBehavior* bhvSpring, NSUInteger idx, BOOL *stop) {
        ...
        CGFloat distTouch = fabsr(posTouch.y - bhvSpring.anchorPoint.y);
...
- (void)__navi_pantransition {
    CGFloat prg = fabsr(self.panTransition.translation.x / CGRectGetWidth(self.view.bounds));
...
- (id)initWithFrame:(CGRect)frame {
    ...
    self.placeholderColor = [UIColor colorWithRGB:0x11EA22];
...
- (void)dealloc {
    ...
    ZERO_RELEASE(_placeholderColor);
...
- (void)setPlaceholder:(NSString *)placeholder {
    NSAttributedString *str = [[NSAttributedString alloc] initWithString:placeholder
...
+ (UIImage*)stretchImage:(NSString *)name {
    return [self stretchImage:name anchorPoint:kCGAnchorPointCC];
}
...
+ (UIImage*)loadImageNamed:(NSString *)name {
    ...
    if (path.notEmpty == NO) {
        if (name.notEmpty)
            WARN("没有找到对应的图片 %s", name.UTF8String);
        return nil;
    }
...
- (void)didPresentActionSheet:(UIActionSheet *)actionSheet {
    // 如果点击了背景，也可以关闭 as
    [[UIKit shared].signals connect:kSignalTouchesBegan withSelector:@selector(__ase_any_touch) ofTarget:self].eps = 1;
}
...
- (UIMenuItem*)addItem:(NSString *)title {
    UIMenuItem* mi = [[UIMenuItem alloc] initWithTitle:title action:@selector(pass)];
...
SIGNAL_ADD(kSignalSelectionChanged)
[self.signals connect:kSignalValueChanged withSelector:@selector(__cb_valuedchanged:) ofTarget:self];

SIGNALS_END

- (void)__cb_valuedchanged:(SSlot*)s {
    [self.signals emit:kSignalSelectionChanged withResult:self.date];
}

- (NSTime*)time {
    return [NSTime timeWithDate:self.date];
}

- (void)setTime:(NSTime*)time animated:(BOOL)animated {
    [self setDate:time.date animated:animated];
}

@end

@implementation UIDatePickerExt
...
[self.signals settingForSignal:kSignalDeviceShaking].eps = 1;
[self.signals settingForSignal:kSignalDeviceShaked].eps = 1;
[self.signals settingForSignal:kSignalDeviceVibrate].eps = 1;
...
BOOL UIPanelPatternTypeHasBottomEdge(UIPanelPatternType ty) {
    return ty == kUIPanelPatternTypeHeader || ty == kUIPanelPatternTypeBody;
}
