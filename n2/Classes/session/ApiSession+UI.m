...
- (void)setScrollView:(UIScrollView *)scrollView {
    ...
    if (scrollView) {
        [self.signals connect:kSignalApiSucceed withSelector:@selector(__snetobj_workdone:) ofTarget:self].priority = kSSlotPriorityHigh;
        [self.signals connect:kSignalApiFailed withSelector:@selector(__snetobj_workstop:) ofTarget:self].priority = kSSlotPriorityHigh;
    }
...
- (void)__snetobj_workdone:(SSlot*)s {
    ...
    [scroll.identifierBottom offsetPosition:CGPointMake(0, scroll.identifierBottom.dockedHeight)];
...
- (void)setScrollView:(UIScrollView *)scrollView {
    ...
    if (scrollView) {
        [self.signals connect:kSignalApiSucceed withSelector:@selector(__snetobjs_workdone:) ofTarget:self].priority = kSSlotPriorityHigh;
        [self.signals connect:kSignalApiFailed withSelector:@selector(__snetobjs_workstop:) ofTarget:self].priority = kSSlotPriorityHigh;
    }
}
